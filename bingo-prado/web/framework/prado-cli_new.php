 ...
 * @version $Id: prado-cli.php 1954 2012-04-04 07:39:21Z daniel $
 * @since 3.1
 */
...
if (class_exists('PHP_Shell_Commands', false)) {
    ...
    class PHP_Shell_Extensions_ActiveRecord implements PHP_Shell_Extension {
        ...
        public function register() {
            ...
            if (Prado::getApplication() !== null) {
                $cmd->registerCommand('#^generate#', $this, 'generate', 'generate <table> <output> [soap]', 'Generate Active Record skeleton for <table> to <output> file using' . PHP_EOL .
    ...
    //--------------------------------------------------------------------------------------------------------
    class PHP_Shell_Extensions_ActiveRecordAll implements PHP_Shell_Extension {

        public function register() {
            $cmd = PHP_Shell_Commands::getInstance();
            if (Prado::getApplication() !== null) {

                $cmd->registerCommand('#^generateAll#', $this, 'generateAll', 'generateAll <output> [soap] [overwrite] [prefix] [postfix]', 'Generate Active Record skeleton for all Tables to <output> file using' . PHP_EOL .
                        '    application.xml in [directory]. May also generate [soap] properties.' . PHP_EOL .
                        '    Generated Classes are named like the Table with optional [Prefix] and/or [Postfix]. [Overwrite] is used to overwrite existing Files.');
            }
        }

        public function generateAll($l) {
            $inputs = explode(" ", trim($l));
            if (count($inputs) > 1) {
                $cmd = new PradoCommandLineActiveRecordGenAll;
                $cmd->performAction($inputs);
            } else {
                echo "\n    Usage: generateAll Application.path \n";
            }
        }
    ...
    $__shell_exts->registerExtensions(array(
        "active-recordAll" => new PHP_Shell_Extensions_ActiveRecordAll,
