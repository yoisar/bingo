...
class TAuthManager extends TModule
{
     ...
     *
     * @param
     *            TXmlElement configuration for this module, can be null
    ...
    public function init($config)
    {
        ...
        $application->attachEventHandler('OnAuthentication', array(
            $this,
            'doAuthentication'
        ));
        $application->attachEventHandler('OnEndRequest', array(
            $this,
            'leave'
        ));
        $application->attachEventHandler('OnAuthorization', array(
            $this,
            'doAuthorization'
        ));
     ...
     *
     * @param
     *            string|IUserManager the user manager module ID or the user manager object
     ...
     *
     * @param
     *            mixed sender of the Authentication event
     * @param
     ...
     *
     * @param
     *            mixed sender of the Authorization event
     * @param
     *            mixed event parameter
     */
     ...
     *
     * @param
     *            string the name of the session variable storing return URL.
     */
     ...
     *
     * @param
     *            boolean whether to allow remembering login so that the user logs on automatically next time. Users have to enable cookie to make use of this feature.
     * @since 3.1.1
     */
     ...
     *
     * @param
     *            mixed parameter to be passed to OnAuthenticate event
     ...
     *
     * @param
     *            mixed parameter to be passed to OnAuthExpire event.
     */
     ...
     *
     * @param
     *            mixed parameter to be passed to OnAuthorize event
     */
    public function onAuthorize($param)
    {
        ...
        if (! $application->getAuthorizationRules()->isUserAllowed($application->getUser(), $application->getRequest()
            ->getRequestType(), $application->getRequest()
            ->getUserHostAddress())) {
     ...
     *            string password
     * @param
     *            integer number of seconds that automatic login will remain effective. If 0, it means user logs out when session ends. This parameter is added since 3.1.1.
     * @return boolean if login is successful
     */
    public function login($username, $password, $expire = 0)
    {
        // MyUtil::echoDebug('login...');
        // MyUtil::echoDebug("validating user $username clave $password...");
        if ($this->_userManager->validateUser($username, $password)) {
            // MyUtil::echoDebug('getting user...');
            if (($user = $this->_userManager->getUser($username)) === null)
                return false;
            $this->updateSessionUser($user);
            $this->getApplication()->setUser($user);
            ...
            // MyUtil::echoDebug('returning false...');
            return false;
    }
